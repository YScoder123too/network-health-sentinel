      ...
      <main style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative", zIndex: 1 }}>
        ...
        <section style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "rgba(0,0,0,0.8)" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "32px 40px", display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            {activeView === "upload" && !loading && (
              <div style={{ width: "100%", maxWidth: 520, marginTop: 32, animation: "fadeIn 0.4s ease" }}>
                <div style={{ border: "1px dashed rgba(0,255,65,0.2)", padding: "48px 32px", textAlign: "center", background: "rgba(0,255,65,0.01)", marginBottom: 28, position: "relative", overflow: "hidden", transition: "all 0.2s" }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(0,255,65,0.7)"; e.currentTarget.style.background = "rgba(0,255,65,0.04)"; e.currentTarget.style.boxShadow = "0 0 30px rgba(0,255,65,0.08), inset 0 0 30px rgba(0,255,65,0.03)" }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(0,255,65,0.2)"; e.currentTarget.style.background = "rgba(0,255,65,0.01)"; e.currentTarget.style.boxShadow = "none" }}
                  onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor = "#00ff41"; e.currentTarget.style.background = "rgba(0,255,65,0.07)"; e.currentTarget.style.boxShadow = "0 0 40px rgba(0,255,65,0.15), inset 0 0 40px rgba(0,255,65,0.05)" }}
                  onDragLeave={e => { e.currentTarget.style.borderColor = "rgba(0,255,65,0.2)"; e.currentTarget.style.background = "rgba(0,255,65,0.01)"; e.currentTarget.style.boxShadow = "none" }}
                  onDrop={e => { e.preventDefault(); e.currentTarget.style.borderColor = "rgba(0,255,65,0.2)"; e.currentTarget.style.background = "rgba(0,255,65,0.01)"; e.currentTarget.style.boxShadow = "none"; const f = e.dataTransfer.files?.[0]; if (f) handleFileSelect(f) }}>
                  {/* Scanning line */}
                  <div style={{ position: "absolute", left: 0, right: 0, height: 1, background: "linear-gradient(90deg, transparent, rgba(0,255,65,0.5), transparent)", animation: "scan 2.5s ease-in-out infinite", pointerEvents: "none" }} />
                  {/* Bigger corner brackets */}
                  {[{top:0,left:0},{top:0,right:0},{bottom:0,left:0},{bottom:0,right:0}].map((pos,i) => (
                    <div key={i} style={{ position:"absolute", ...pos, width:20, height:20, borderTop: i<2?"2px solid rgba(0,255,65,0.6)":"none", borderBottom: i>=2?"2px solid rgba(0,255,65,0.6)":"none", borderLeft: i%2===0?"2px solid rgba(0,255,65,0.6)":"none", borderRight: i%2===1?"2px solid rgba(0,255,65,0.6)":"none" }} />
                  ))}
                  <div style={{ fontSize: 44, marginBottom: 12, filter: "drop-shadow(0 0 10px rgba(0,255,65,0.25))" }}>
                    {file ? (isPcap(file) ? "📡" : "📄") : "📂"}
                  </div>
                  <div style={{ fontSize: 9, opacity: 0.2, letterSpacing: 2, marginBottom: 12 }}>AWAITING_INPUT · MAX 2000 ROWS · 7 FEATURE DIMS</div>
                  <div style={{ fontSize: 13, opacity: 0.7, marginBottom: 8 }}>
                    {file ? <><span style={{ color: isPcap(file) ? "#0047ab" : "#00ff41" }}>{file.name}</span>{isPcap(file) ? " — PCAP ready" : " — CSV ready"}</> : <>Drop <code style={{ color: "#00ff41" }}>.csv</code> or <code style={{ color: "#0047ab" }}>.pcap</code></>}
                  </div>
                  <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginTop: 20 }}>
